...
#path : Devices/testdevice1/side1
def training(path):
    seed(1)
    set_random_seed(2)
    ...
    session = tf.Session()
    x = tf.placeholder(tf.float32, shape=[None, img_size, img_size, num_channels], name= path + "_x")

    ## labels
    # maybe.. if we take many models, we should change the name(' y_true' -> 'model name_y_true')
    y_true = tf.placeholder(tf.float32, shape=[None, num_classes], name= path + "_true")
    ...
    y_pred = tf.nn.softmax(layer_fc2, name=path + "_pred")
    ...
    st = total_iterations
    fin = total_iterations + num_iteration
    ...
    for i in range(st, fin):
        ...
        if i % int(data.train.num_examples / batch_size) == 0:
            ...
            saver.save(session, path + '/model/model')
